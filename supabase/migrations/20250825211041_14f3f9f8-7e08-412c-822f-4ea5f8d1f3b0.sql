-- Security hardening: enable RLS and add least-privilege policies for list-related and taxonomy tables
...
-- 5) member_user_ids: user can read their own membership row; admin manages writes
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'member_user_ids' AND policyname = 'member_user_ids_self_or_admin_read'
  ) THEN
    CREATE POLICY "member_user_ids_self_or_admin_read"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'member_user_ids' AND policyname = 'member_user_ids_admin_insert'
  ) THEN
    CREATE POLICY "member_user_ids_admin_insert"
    ON public.member_user_ids
    FOR INSERT
    WITH CHECK (public.is_admin(auth.uid()));
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'member_user_ids' AND policyname = 'member_user_ids_admin_update'
  ) THEN
    CREATE POLICY "member_user_ids_admin_update"
    ON public.member_user_ids
    FOR UPDATE
    USING (public.is_admin(auth.uid()))
    WITH CHECK (public.is_admin(auth.uid()));
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'member_user_ids' AND policyname = 'member_user_ids_admin_delete'
  ) THEN
    CREATE POLICY "member_user_ids_admin_delete"
    ON public.member_user_ids
    FOR DELETE
    USING (public.is_admin(auth.uid()));
  END IF;
